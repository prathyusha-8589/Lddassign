...
struct circ_buf cbuf;//circularbuffers
static wait_queue_head_t _wq; //wait queue where process is started
...
static ssize_t sample_read(struct file *filep,char __user *ubuff,size_t cnt,loff_t *offset){
	...
	wait_event_interruptible(_wq,CIRC_CNT(cbuf.head,cbuf.tail,SIZE)>0);//0 means no data
